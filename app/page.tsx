    ...
    <div className="page-wrapper">
      <style jsx global>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; background: radial-gradient(circle at top left, #0055FF, #000000); background-attachment: fixed; color: white; overflow-x: hidden; }
        ...
        .glass-modal { background: rgba(10, 10, 15, 0.9); border: 1px solid rgba(255, 255, 255, 0.15); padding: 40px; border-radius: 30px; max-width: 900px; width: 90%; text-align: center; position: relative; color: white; box-shadow: 0 50px 100px rgba(0,0,0,0.9); }
      ...
      <section className="section-card card-info">
         <div className="content-container" style={{flexDirection:'column'}}>
            ...
            <div className="info-grid">
               <div className="info-card"><h3 style={{fontSize:'2.5rem', color:'#FFD700', marginBottom:'10px'}}>%100</h3><p>Acımasız Doğruluk. Duygularını kapıda bırak.</p></div>
               <div className="info-card"><h3 style={{fontSize:'2.5rem', color:'#FFD700', marginBottom:'10px'}}>+50K</h3><p>Ağlayan Kullanıcı. Egosu kırılanlar kulübü.</p></div>
               <div className="info-card"><h3 style={{fontSize:'2.5rem', color:'#FFD700', marginBottom:'10px'}}>GPT-4o</h3><p>OpenAI'ın en son teknolojisi ile derin analiz.</p></div>
               <div className="info-card" style={{border:'1px solid rgba(255,215,0,0.3)'}}><h3 style={{fontSize:'2.5rem', color:'#FFD700', marginBottom:'10px'}}>GROK</h3><p>xAI destekli filtresiz, sansürsüz mizah motoru.</p></div>
            </div>
         </div>
      </section>
      ...
      <section className="section-card card-footer">
         <div className="footer-grid">
            <div className="footer-col" style={{gridColumn: 'span 2'}}><h4 style={{fontSize:'2rem', color:'#fff'}}>RoastAI Inc.</h4><p style={{color:'#ccc', lineHeight:'1.6', fontSize:'1.1rem', maxWidth:'400px'}}>İnternetin en karanlık mizah köşesi.</p><p style={{opacity:0.5, marginTop: '30px'}}>© 2026 RoastAI.</p></div>
            <div className="footer-col"><h4>Yasal</h4><a onClick={() => metinAc("Kullanım Şartları")}>Kullanım Şartları</a><a onClick={() => metinAc("Gizlilik Politikası")}>Gizlilik Politikası</a><a onClick={() => metinAc("Rıza Metni")}>Rıza Metni</a></div>
            <div className="footer-col"><h4>İletişim</h4><a href="https://x.com/cagrium" target="_blank">@cagrium (Twitter)</a><a href="mailto:hello@roastai.com">hello@roastai.com</a></div>
         </div>
      </section>
        ...
        <div className="modal-overlay">
           <div className="glass-modal animate-bounce-in" style={{maxWidth:'900px', padding:'50px'}}> 
              ...
              <div className="pricing-grid">
                 <div className="price-card"><div className="price-header"><div className="price-title">BAŞLANGIÇ</div><div className="price-amount">$2.99</div></div><p className="price-desc">5 Profil Analizi</p><button className="buy-btn" onClick={satinAlSimulasyon}>SATIN AL</button></div>
                 <div className="price-card"><div className="price-header"><div className="price-title">POPÜLER</div><div className="price-amount">$7.99</div></div><p className="price-desc">25 Profil Analizi</p><button className="buy-btn" onClick={satinAlSimulasyon}>SATIN AL</button></div>
                 <div className="price-card" style={{background:'rgba(255,215,0,0.15)', borderColor:'#FFD700'}}><div className="special-badge">❄️ YILBAŞI ÖZEL</div><div className="price-header"><div className="price-title" style={{color:'#FFD700'}}>SINIRSIZ</div><div style={{display:'flex', alignItems:'center', gap:'8px'}}><span className="strike-price">$14.99</span><span className="price-amount large">$9.99</span></div></div><p className="price-desc">Tek Seferlik Ödeme</p><button className="buy-btn gold" onClick={satinAlSimulasyon}>SATIN AL</button></div>
              </div>
           </div>
        </div>
      )}

      {nasilCalisirAcik && (<div className="modal-overlay"><div className="glass-modal"><button onClick={() => setNasilCalisirAcik(false)} style={{position:'absolute', top:'20px', right:'30px', background:'none', color:'white', fontSize:'2rem', fontWeight:'bold', cursor:'pointer'}}>✕</button><h2 style={{fontSize:'2rem', fontWeight:'900', marginBottom:'30px'}}>SİSTEM</h2><div style={{textAlign:'left', display:'flex', flexDirection:'column', gap:'20px'}}><div style={{display:'flex', alignItems:'center', gap:'15px'}}><span style={{fontSize:'2rem'}}>🕵️‍♂️</span><div><h4 style={{color:'#FFD700'}}>Veri Tarama</h4><p style={{fontSize:'0.9rem', color:'#ccc'}}>Profilini saniyeler içinde tarıyoruz.</p></div></div><div style={{display:'flex', alignItems:'center', gap:'15px'}}><span style={{fontSize:'2rem'}}>🧠</span><div><h4 style={{color:'#FFD700'}}>Analiz</h4><p style={{fontSize:'0.9rem', color:'#ccc'}}>GPT-4o zayıf noktanı bulur.</p></div></div><div style={{display:'flex', alignItems:'center', gap:'15px'}}><span style={{fontSize:'2rem'}}>🔥</span><div><h4 style={{color:'#FFD700'}}>Roast</h4><p style={{fontSize:'0.9rem', color:'#ccc'}}>En acımasız gerçekleri yüzüne vururuz.</p></div></div></div></div></div>)}

      {girisAcik && (<div className="modal-overlay"><div className="glass-modal"><button onClick={() => setGirisAcik(false)} style={{position:'absolute', top:'20px', right:'30px', background:'none', color:'white', fontSize:'2rem', fontWeight:'bold', cursor:'pointer'}}>✕</button><h2 style={{fontSize:'2rem', fontWeight:'900', marginBottom:'10px'}}>{isLoginMode ? "Giriş Yap" : "Kayıt Ol"}</h2><input style={{background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', color:'white', padding:'15px', width:'100%', borderRadius:'12px', marginBottom:'15px'}} type="email" placeholder="E-posta" onChange={(e) => setUserEmail(e.target.value)} /><input style={{background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', color:'white', padding:'15px', width:'100%', borderRadius:'12px', marginBottom:'15px'}} type="password" placeholder="Şifre" /><button className="btn-premium" style={{marginTop:'10px', fontSize:'1rem'}} onClick={girisYapSimulasyon}>{isLoginMode ? "Giriş Yap ->" : "Hesap Oluştur ->"}</button><p style={{marginTop:'20px', cursor:'pointer', color:'#FFD700'}} onClick={() => setIsLoginMode(!isLoginMode)}>{isLoginMode ? "Hesabın yok mu? Kayıt Ol" : "Giriş Yap"}</p></div></div>)}

      {yasalModal.acik && (<div className="modal-overlay"><div className="glass-modal"><button onClick={() => setYasalModal({...yasalModal, acik: false})} style={{position:'absolute', top:'20px', right:'30px', background:'none', color:'white', fontSize:'2rem', fontWeight:'bold', cursor:'pointer'}}>✕</button><h2 style={{fontSize:'1.8rem', fontWeight:'900', marginBottom:'20px', color:'#FFD700'}}>{yasalModal.baslik}</h2><p style={{lineHeight:'1.6', color:'#ccc', textAlign:'left', whiteSpace:'pre-wrap'}}>{yasalModal.icerik}</p></div></div>)}

      {sonuc && (<div className="modal-overlay"><div className="x-card"><button onClick={() => setSonuc("")} style={{position:'absolute', top:'10px', right:'15px', background:'none', color:'#71767b', fontSize:'1.5rem', fontWeight:'bold', cursor:'pointer'}}>✕</button><div className="x-header"><div className="x-user"><div className="x-avatar">🤖</div><div className="x-names"><div className="x-name">RoastAI Bot <span style={{color:'#1d9bf0', marginLeft:'4px'}}>✔</span></div><div className="x-handle">@roast_ai_bot</div></div></div><div className="x-logo">𝕏</div></div><div className="x-content">{sonuc}</div><div style={{color:'#71767b', fontSize:'15px', marginBottom:'15px'}}>3:24 PM · Jan 29, 2026 · <span style={{color:'white', fontWeight:'bold'}}>1.2M</span> Views</div><div className="x-metrics"><div className="x-metric">💬 2.1K</div><div className="x-metric">↻ 15K</div><div className="x-metric">♥ 85K</div><div className="x-metric" onClick={paylas}>Share</div></div></div></div>)}
    </div>
  );
}